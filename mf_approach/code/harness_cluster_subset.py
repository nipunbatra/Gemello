...
#for appliance in ['hvac']:
    ...
    for num_homes in range(5, 55, 5):
        for home in X_matrix.index:
            OFILE = "%s/%s_%d_%d.out" % (SLURM_OUT, appliance, home, num_homes)
            EFILE = "%s/%s_%d_%d.err" % (SLURM_OUT, appliance, home, num_homes)
            SLURM_SCRIPT = "%s_%d_%d.pbs" %(appliance, home, num_homes)
            CMD = 'python train_all_homes_test_all_homes_subset.py %s %d %d %d' %(appliance, home, home_var, num_homes)
            lines = []
            lines.append("#!/bin/sh\n")
